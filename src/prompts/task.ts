...
export async function collectTaskInfo(): Promise<TaskInput> {
  ...
  const client = await promptSelectOrCustom(
    'Select client:',
    availableClients,
    'Clients'
  );
 ...
 */
export function displayTaskSummary(input: TaskInput, rate: number, date: string, time: string): void {
  /* v8 ignore start */
  console.log(chalk.blue.bold('\n📋 === Task Summary ==='));
  ...
  console.log(chalk.white(`Total: ${chalk.green.bold(`$${(input.hoursWorked * rate).toFixed(2)}`)}`));
